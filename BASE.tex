...
Uma exemplificação é dada a partir de um trecho de código como mostra abaixo, onde seu vocabulário extraído, está numa representação XML(eXtensible Markup Language)  
...
\quad A fim de analisar e extrair o vocabulário de hardware pertencentes a projetos descritos em SystemVerlog desenvolvemos o ferramental \textit{Hardware Vocabulary Tool}, que
	
	
	Afim de calcularmos uma porcentagem de extracão que melhor represente a eficiência do software proposto, foi elaborado um design genérico com todas as estruturas possíveis em SistemVerilog. Os resultados obtidos são apresentados na tabela abaixo:
...
\textcolor{gray}{/* the function nextAdrres return the}

\textcolor{gray}{next adrres to register PC}

\textcolor{gray}{*/}

\textcolor{blue}{function logic}[\textcolor{green}{7}:\textcolor{green}{0}] nextAdrres(\\\textcolor{blue}{\qquad\quad input logic}[\textcolor{green}{7}:\textcolor{green}{0}] registerCounter,\\ \textcolor{blue}{\qquad\quad input logic}[\textcolor{green}{7}:\textcolor{green}{0}] registerAdress);\\\\
\textcolor{blue}{\qquad return} registerCounter + registerAdress \textcolor{blue}{$ << $}\textcolor{green}{ 2} ;\\
\textcolor{blue}{\quad endfunction}: nextAdrres
